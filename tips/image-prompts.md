...
## 7. サムネ（最新版・オタク感・文字入り・1.91:1）「推しをAI役員に雇った件について」
> コンセプト：推し活する主役（オタク女子）＋“推しが具現化したAI役員”。著作権ゼロ・テーマ直球・文字込み1発。
> サイズ：1.91:1（1600×838px推奨）。安全マージン込みで端切れ対策済み。

**合体プロンプト（最新版・コピペ用）**
```
Eye-catching Japanese info-product thumbnail illustration, wide 1.91:1 aspect ratio, bold poster style, fun otaku fan-culture (oshi-katsu) vibe, leave safe margins on all sides and keep all key elements within the central safe zone. In the center foreground, an enthusiastic young Japanese otaku woman with big sparkling joyful eyes and a wide happy grin, casual cute fan outfit with badges, holding a glowing penlight, surrounded by floating pink hearts and sparkles as if cheering for her favorite characters. Around and behind her float five distinct glowing holographic AI executive avatars — charming ORIGINAL anime-style characters brought to life as her dream executive team, each a different neon color with a clear role icon glowing above its head: a golden crown (CEO), a headset (customer support), a wrench (craftsman), a sparkling upward chart (sales), and a calculator (finance). Background: a cozy otaku room blending into a bright modern office, soft blurred silhouettes of generic figures and posters on shelves, a glowing rising success arrow and a light-bulb idea icon, neon glow, cinematic rim lighting, vibrant high-saturation modern anime illustration, lively but clean and readable composition. At the very top, large bold Japanese title on two lines: big first line 「推しをAI役員に雇った」 and a smaller second line 「件について」, thick navy gothic font with a red accent color; just below it a small red ribbon banner with white text reads 「設立のやり方、ぜんぶ公開します」. Crisp legible kanji, accurate Japanese typography, strong contrast, professional polished thumbnail. Avoid: realistic photo, 3d render, garbled or gibberish text, misspelled kanji, watermark, deformed hands, extra fingers, low quality, cluttered edges, any recognizable or existing anime / game / idol / celebrity character (use only original designs).
```

**タイトルを「話」に戻す場合**：末尾の `big first line ... 「件について」` を `「推しをAI役員に雇った話」 on one line` に置換。
**日本語が崩れたら**：`「…」`まわりを消して画像だけ生成 → Canva等で文字を乗せる（極太ゴシック・紺＋赤差し色）。
